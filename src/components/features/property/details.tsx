...
export default function Details({ propertyId, onBack }: DetailsProps) {
    ...
    async function fetchPropertyDetails() {
      ...
      try {
        ...
        const { data, error } = await supabase
          .from("properties")
          .select(
            `*, states_regions(name_en, name_mm), townships(name_en, name_mm)`,
          )
          .eq("id", propertyId)
          .single();
  ...
  if (property.video_url) {
    mediaDataset.push(property.video_url);
  } else if (property.video) {
    mediaDataset.push(property.video);
  }
  ...
  if (mediaDataset.length === 0) {
    mediaDataset = [DEFAULT_IMAGE];
  }
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      >
            ...
            {mediaDataset.map((mediaUrl: string, idx: number) => {
              ...
              return (
                <TouchableOpacity
                  key={idx}
                  activeOpacity={0.9}
                  onPress={() => handleImagePress(mediaUrl)}
                >
                  <Image
                    source={{ uri: mediaUrl }}
        ...
        <View className="bg-white rounded-t-[32px] -mt-6 p-6 shadow-xl shadow-slate-200/50 min-h-[500px]">
      ...
      <View className="px-6 py-4 bg-white border-t border-slate-100 flex-row items-center gap-4 shadow-2xl">
        <View className="flex-1">
          <Text className="text-slate-400 text-xs font-bold uppercase tracking-wider">
            {isBurmese ? "စုစုပေါင်း" : "Total Amount"}
          </Text>
        ...
        <TouchableOpacity
          onPress={() =>
            handleCallPress(property.search_value || property.phone)
          }
...
// ── 💡 ဗီဒီယိုရဲ့ ညာဘက်ထောင့်မှာပဲ လေးထောင့်ကွက်လေး တင်ပေးထားသော COMPONENT ──
function VideoItem({
  videoUrl,
  isActive,
}: {
  ...
  return (
    <View style={styles.videoWrapper}>
      ...
      {/* 💡 ညာဘက်အောက်ခြေနားလေးတင် ပေါ်နေမယ့် ဇိမ်ခံ လေးထောင့်ကွက် Fullscreen Icon */}
      <TouchableOpacity
        ...
        <View style={styles.fullVideoContainer}>
          ...
          <TouchableOpacity
            onPress={() => {
              ...
              // 💡 ခဏကြာမှ ပြန်ဆော့စေရန် (UI Thread ရှင်းသွားအောင်)
              setTimeout(() => {
                player.play();
              }, 100);
            }}
...
const styles = StyleSheet.create({
  ...
  // 💡 ဗီဒီယို Container နှင့် ညာဘက်ထောင့်က လေးထောင့်ကွက် Style များ
  videoWrapper: {
    width: SCREEN_WIDTH,
    height: 320,
    backgroundColor: "#000",
  ...
  squareFullBtn: {
    position: "absolute",
    bottom: 50, // Native Play/Pause Bar တွေနဲ့ မလွတ်ရင် 16 သို့မဟုတ် 20 ကို လိုသလို အနည်းငယ် တိုး/လျှော့ ချိန်နိုင်ပါတယ်
    right: 14,
    backgroundColor: "rgba(0, 0, 0, 0.5)", // မှန်အမည်းကွက် ပုံစံလေး
    width: 36,
    height: 36,
    borderRadius: 6, // Square ဆန်ဆန်လေး ဖြစ်အောင် လေးထောင့်ကွက် ပုံစံ
    justifyContent: "center",
    alignItems: "center",
    borderWidth: 0.5,
    borderColor: "rgba(255, 255, 255, 0.3)",
    zIndex: 9999, // ဗီဒီယိုရဲ့ ထိပ်ဆုံးမှာ ပေါ်နေစေရန်
  },
  fullVideoContainer: {
    flex: 1,
    backgroundColor: "#000",
