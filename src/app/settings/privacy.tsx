...
export default function PrivacySettingsScreen() {
  ...
  const items = [
    { icon: <Eye size={20} color="#22c55e" />, label: "Show online status", value: showOnline, set: setShowOnline },
    { icon: <Shield size={20} color="#22c55e" />, label: "Show phone number", value: showPhone, set: setShowPhone },
    { icon: <UserX size={20} color="#22c55e" />, label: "Private profile", value: !profilePublic, set: (v: boolean) => setProfilePublic(!v) },
  ];
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-black">
      <View className="px-4 py-4 flex-row items-center bg-white dark:bg-gray-900 border-b border-slate-100 dark:border-gray-800">
        ...
        <Text className="text-lg font-bold text-slate-800 dark:text-gray-100 ml-4">Privacy & Security</Text>
      </View>
      <ScrollView className="flex-1 px-6" showsVerticalScrollIndicator={false}>
        <View className="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border border-slate-100 dark:border-gray-800 mt-6">
          <View className="px-5 py-3 border-b border-slate-100 dark:border-gray-800">
            <Text className="text-slate-500 dark:text-gray-400 text-xs font-semibold uppercase tracking-wider">Privacy Controls</Text>
          </View>
          {items.map((item, i) => (
            <View
              key={i}
        ...
        <View className="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border border-slate-100 dark:border-gray-800 mt-6">
          <View className="px-5 py-3 border-b border-slate-100 dark:border-gray-800">
            <Text className="text-slate-500 dark:text-gray-400 text-xs font-semibold uppercase tracking-wider">Security</Text>
          </View>
          <TouchableOpacity className="flex-row items-center gap-3 px-5 py-4 active:bg-slate-50 dark:active:bg-gray-700">
            ...
            <Text className="text-slate-700 dark:text-gray-200 font-semibold">Change Password</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
